...
// the hook in the forntend will call this endpoint to update the restaurant
...
//             res.status(401).send();// send 401 if the user is not the owner of the restaurant
//             return;
//         }
//         order.status = status;
//         await order.save();
//         res.status(200).send(order);// send the updated order to the frontend to update the UI 
